...
// Find your updateWorkOrderStatus function and ensure it includes SMS webhook
exports.updateWorkOrderStatus = async (req, res) => {
    try {
        ...
        // Find the work order with all necessary data
        ...
        console.log(`📋 Work order ${workOrder.job_no}: ${previousStatus} → ${status}`);
        ...
        // Create status update record if StatusUpdate model exists
        try {
            if (db.statusUpdate) {
        ...
        // AFTER sending response, try to send SMS webhook
        console.log('🚀 Attempting to send SMS webhook...');
        ...
        try {
            ...
            if (smsResult.success) {
                console.log(`✅ SMS webhook sent successfully for work order ${workOrder.job_no}`);
                console.log('SMS Response:', smsResult);
            } else {
                console.log(`⚠️  SMS webhook failed for work order ${workOrder.job_no}`);
                console.log('SMS Error:', smsResult.reason || smsResult.error);
            }

        } catch (smsError) {
            // Log the error but don't affect the main flow
            console.error('❌ Error sending SMS webhook:', smsError);
        }

        // Also try to create in-app notifications if available
        try {
